...
impl MtpSender {
    ...
    /// For FakeTLS: parse TLS records from `read_buf`, decrypt AppData into `plain_buf`.
    fn ingest_faketls_records(&mut self) -> Result<(), InvocationError> {
        let FrameKind::FakeTls {
            cipher, plain_buf, ..
        } = &self.frame_kind
        else {
            return Ok(());
        ...
        let mut offset = 0usize;
        while offset + 5 <= self.read_tail {
            let rtype = self.read_buf[offset];
            let len =
                u16::from_be_bytes([self.read_buf[offset + 3], self.read_buf[offset + 4]]) as usize;
            if offset + 5 + len > self.read_tail {
                break;
            }
            let mut payload = self.read_buf[offset + 5..offset + 5 + len].to_vec();
            offset += 5 + len;
            match rtype {
                0x14 => {}
                0x17 => {
                    ...
                    if let Ok(mut pb) = plain_buf.try_lock() {
                        pb.extend_from_slice(&payload);
                    }
                }
                0x15 => {
                    return Err(InvocationError::Deserialize(
                        "FakeTLS: TLS Alert while reading".into(),
                    ));
                }
                other => {
    ...
    /// Consume peeled bytes from either FakeTLS plain_buf or the raw read buffer.
    fn consume_peeled(&mut self, consumed: usize) {
        if let FrameKind::FakeTls { plain_buf, .. } = &self.frame_kind {
            if consumed > 0 {
                if let Ok(mut pb) = plain_buf.try_lock() {
                    if consumed <= pb.len() {
                        pb.drain(..consumed);
    ...
    fn frame_encode(&self, data: &[u8]) -> Vec<u8> {
        match &self.frame_kind {
            ...
            FrameKind::FakeTls {
                ...
                if let Ok(mut init) = pending_init.try_lock() {
                    if let Some(nonce) = init.take() {
                ...
                let mut out = Vec::new();
                if first_packet.swap(false, std::sync::atomic::Ordering::SeqCst) {
    ...
    fn peel_one(&self, offset: usize) -> Peel {
        if let FrameKind::FakeTls { plain_buf, .. } = &self.frame_kind {
            let Ok(pb) = plain_buf.try_lock() else {
                return Peel::Incomplete;
            };
            let buf = &pb[offset..];
            return match peel_intermediate(buf, offset) {

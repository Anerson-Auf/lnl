...
pub async fn send_frame(
    ...
    match kind {
        ...
        FrameKind::FakeTls {
            ...
            // PaddedIntermediate + Obfuscated2, then wrap in FakeTLS AppData.
            let mut pad_len_buf = [0u8; 1];
            ferogram_crypto::fill_random(&mut pad_len_buf);
            let pad_len = (pad_len_buf[0] & 0x0f) as usize;
            let total_payload = data.len() + pad_len;
            ...
            if let Some(init) = pending_init.lock().await.take() {
                wire.extend_from_slice(&init);
            }
            wire.extend_from_slice(&frame);

            if first_packet.swap(false, std::sync::atomic::Ordering::SeqCst) {
                crate::faketls::write_ccs(stream).await?;
            }
            crate::faketls::write_app_data(stream, &wire).await?;
            Ok(())
        }
    }
}
...
pub async fn recv_frame_plain<T: Deserializable>(
    ...
    let raw = match kind {
        ...
        FrameKind::FakeTls {
            ...
            // Decrypt TLS AppData into plain_buf, then peel one Intermediate frame.
            loop {
                let mut guard = plain_buf.lock().await;
                if guard.len() >= 4 {
                    let total_len = u32::from_le_bytes(guard[..4].try_into().unwrap()) as usize;
                    if total_len > 0 && total_len <= 1 << 24 && guard.len() >= 4 + total_len {
                        let mut frame = guard.split_off(4);
                        let rest = frame.split_off(total_len);
                        *guard = rest;
                        // Strip Intermediate padding (same heuristic as PaddedIntermediate).
                        if frame.len() >= 24 {
                            let pad = (frame.len() - 24) % 16;
                ...
                drop(guard);
                let mut chunk = crate::faketls::read_app_data(stream).await?;
                cipher.lock().await.decrypt(&mut chunk);
                plain_buf.lock().await.extend_from_slice(&chunk);
            }
        }
    };
    if raw.len() < 20 {

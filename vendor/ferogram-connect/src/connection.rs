...
pub enum FrameKind {
    ...
    /// FakeTLS framing (`0xEE` MTProxy) over PaddedIntermediate + Obfuscated2.
    FakeTls {
        cipher: std::sync::Arc<tokio::sync::Mutex<ferogram_crypto::ObfuscatedCipher>>,
        /// Need ChangeCipherSpec before the first Application Data record.
        first_packet: std::sync::Arc<std::sync::atomic::AtomicBool>,
        /// 64-byte Obfuscated2 init, prepended to the first AppData payload.
        pending_init: std::sync::Arc<tokio::sync::Mutex<Option<[u8; 64]>>>,
        /// Decrypted Intermediate bytes waiting to be peeled (MtpSender).
        plain_buf: std::sync::Arc<tokio::sync::Mutex<Vec<u8>>>,
    },
}
    ...
    async fn apply_transport_init(
        ...
        match transport {
            ...
            TransportKind::FakeTls { secret, domain } => {
                // Real FakeTLS handshake (ClientHello with timestamp XOR, then
                // ServerHello+CCS+AppData). After that, traffic is Obfuscated2
                // PaddedIntermediate inside TLS Application Data records — same
                // as Telegram Desktop / gotd / mtg.
                crate::faketls::handshake(&mut stream, secret, domain).await?;
                let (nonce, cipher) =
                ...
                Ok((
                    ...
                    FrameKind::FakeTls {
                        cipher: cipher_arc,
                        first_packet: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(true)),
                        pending_init: std::sync::Arc::new(tokio::sync::Mutex::new(Some(nonce))),
                        plain_buf: std::sync::Arc::new(tokio::sync::Mutex::new(Vec::new())),
                    },
                ))
            }
            TransportKind::Http => {
